//! HAL driver contract tests: Rust/Swift shared-memory protocol, realtime-path
//! restrictions, and daemon reconfiguration invariants.
//!
//! These tests sniff source text across the split checkouts (Swift HAL driver
//! under `sotf-systemwide/swift`, daemon under `sotf-systemwide/crates/daemon`,
//! engine under `sotf-daw/crates/sotf-engine`). They moved here from
//! `sotf-daw`'s `driver-hal` test targets because `sotf-daw` must stay
//! independent of its siblings, while this workspace already assumes the
//! `all_of_sotf` sibling layout (`../sotf-daw`, `../sotf` path dependencies).
//! A standalone clone without those siblings cannot run this target.
use std::fs;
...
fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(3)
        .expect("sotf-daemon manifest should be nested three levels below the all_of_sotf root")
        .to_path_buf()
}
...
fn decoder_retries_hal_reader_after_late_shared_memory_creation() {
    let source = format!(
        "{}\n{}",
        read_repo_file("sotf-daw/crates/sotf-engine/src/engine/decoder_thread/decoder_state.rs"),
        read_repo_file("sotf-daw/crates/sotf-engine/src/engine/decoder_thread/consts.rs")
    );
...
fn swift_encrypted_transport_is_disabled_on_realtime_paths() {
    let shared_memory =
        read_repo_file("sotf-systemwide/swift/driver-hal/Sources/SharedMemory.swift");
    let driver = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/SotFHALDriver.swift");
    let daemon = read_repo_file("sotf-systemwide/crates/daemon/bin/sotf_daemon/audio_daemon.rs");
...
fn swift_hal_callback_does_not_retry_shared_memory_initialization() {
    let source = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/SotFHALDriver.swift");
...
fn swift_reports_legal_zero_time_stamp_period() {
    let source = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/SotFHALDriver.swift");
    let timing = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/Timing.swift");
...
fn swift_shared_memory_is_open_only_for_restricted_hal_process() {
    let source = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/SharedMemory.swift");
...
fn swift_interleaved_loopback_publishes_once_per_channel_block() {
    let source = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/RingBuffer.swift");
...
fn swift_maintenance_uses_mapping_generation_handoff_for_active_clients() {
    ...
    let swift_tests = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/Tests.swift");
...
fn configbar_runs_launchctl_on_serial_lifecycle_queue() {
    let source = read_repo_file("sotf-systemwide/swift/configbar/src/ConfigBar.swift");
...
fn swift_hal_supports_daemon_requested_channel_counts_up_to_32() {
    let hal_source = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/SotFHALDriver.swift");
    let shm_source = read_repo_file("sotf-systemwide/swift/driver-hal/Sources/SharedMemory.swift");
...
fn swift_hal_encryption_tests_cover_crypto_and_realtime_rejection() {
    // Moved from sotf-daw's driver-hal hal_passthrough_tests: the Swift
    // source lives in this workspace, so the contract check lives here too.
    let swift_tests = include_str!("../../../swift/driver-hal/Sources/Tests.swift");
